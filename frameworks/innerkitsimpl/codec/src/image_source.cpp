...
static bool CopyRGBAToSurfaceBuffer(const DecodeContext& context, sptr<SurfaceBuffer>& sb, PlImageInfo plInfo)
{
    ...
    uint64_t srcStride = static_cast<uint32_t>(plInfo.size.width * NUM_4);
...
static uint32_t AiSrProcess(sptr<SurfaceBuffer> &input, DecodeContext &aisrCtx)
{
    ...
    if (res != VPE_ERROR_OK) {
        ...
        aisrCtx.outInfo.size.width = static_cast<int32_t>(output->GetSurfaceBufferWidth());
        aisrCtx.outInfo.size.height = static_cast<int32_t>(output->GetSurfaceBufferHeight());
