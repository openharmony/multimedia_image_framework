...
{
    string formatResult;
    uint32_t ret = GetEncodedFormat(sourceInfo_.encodedFormat, formatResult);
    if (ret == SUCCESS && formatResult != InnerFormat::RAW_FORMAT) {
        isAstc_ = false;
    } else if (IsASTCSource()) {
