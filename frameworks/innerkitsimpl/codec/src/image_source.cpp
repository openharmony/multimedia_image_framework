...
unique_ptr<PixelMap> ImageSource::CreatePixelMapExtended(uint32_t index, const DecodeOptions &opts, uint32_t &errorCode)
{
    ...
    DecodeContext context = DecodeImageDataToContext(index, info, plInfo, errorCode);
...
uint32_t ImageSource::CreatExifMetadataByImageSource(bool addFlag)
{
    ...
    sourceStreamPtr_->Seek(0);
    uint32_t readSize = 0;
    bool retRead = sourceStreamPtr_->Read(copySize, tmpBuffer.get(), copySize, readSize);
    sourceStreamPtr_->Seek(0);
    if (!retRead) {
    ...
    auto metadataAccessor = MetadataAccessorFactory::Create(tmpBuffer.get(), copySize);
    if (metadataAccessor == nullptr) {
