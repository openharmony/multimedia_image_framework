...
XMP_Uns32 XMPBuffer_IO::GetCurrentSize() const
{
    ...
    if (data.size() > std::numeric_limits<XMP_Uns32>::max()) {
        XMP_Throw("XMPBuffer_IO::GetCurrentSize, size overflow", kXMPErr_ExternalFailure);
    }
