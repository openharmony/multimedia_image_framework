...
#include "image_fwk_create_astc_pixelmap_fuzzer.h"
...
void CreateAstcPixelmapFuzzTest(const uint8_t *data, size_t size)
{
    ...
    std::unique_ptr<Media::PixelMap> input = GetPixelMap(dataIn, size, PixelFormat::ASTC_6x6);
    if (input == nullptr) {
        free(dataIn);
        return;
    }
    std::unique_ptr<Media::PixelMap> input = GetPixelMap(dataIn, size, PixelFormat::ASTC_8x8);
    if (input == nullptr) {
        free(dataIn);
        return;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::Media::CreateAstcPixelmapFuzzTest(data, size);
    return 0;
}