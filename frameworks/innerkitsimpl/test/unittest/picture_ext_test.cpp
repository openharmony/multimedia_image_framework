...
static constexpr int32_t DEFAULT_BUFF_SIZE = 25 * 1024 * 1024;
static constexpr int32_t DEFAULT_QUALITY = 98;
...
{
    size_t length = IMAGE_JPEG_SRC.size();
    char filePath[length + 1];
    if (strcpy_s(filePath, sizeof(filePath), IMAGE_JPEG_SRC.c_str()) != EOK) {
        return nullptr;
    }
...
{
    size_t length = IMAGE_INPUT_JPEGHDR_PATH.size();
    char filePath[BUFFER_SIZE];
    if (strcpy_s(filePath, sizeof(filePath), IMAGE_INPUT_JPEGHDR_PATH.c_str()) != EOK) {
        return nullptr;
    }
    OH_ImageSourceNative *source = nullptr;
