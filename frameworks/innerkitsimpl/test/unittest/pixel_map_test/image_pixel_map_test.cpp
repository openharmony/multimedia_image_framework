...
{
    GTEST_LOG_(INFO) << "ImagePixelMapTest: ImagePixelMap045 Create start";
    ...
    GTEST_LOG_(INFO) << "ImagePixelMapTest: ImagePixelMap045 Create end";
}
...
{
    GTEST_LOG_(INFO) << "ImagePixelMapTest: ImagePixelMap046 Create start";
    const int maxFormat = 4;
    for (int i = 0; i <= maxFormat; i++) {
...
HWTEST_F(ImagePixelMapTest, ImagePixelMap047, TestSize.Level3)
{
    ...
    opts.size.width = 3;
    opts.size.height = 3;
    uint32_t dataLength = opts.size.width * opts.size.height * ARGB8888_BYTES;
    uint32_t* data = new uint32_t[dataLength];
    for (uint32_t i = 0; i < dataLength; i++) {
        data[i] = 0xFFFF0000;
    }
    ...
    opts.size.width = 400;
    opts.size.height = 3;
    dataLength = opts.size.width * opts.size.height * ARGB8888_BYTES;
...
HWTEST_F(ImagePixelMapTest, ImagePixelMap049, TestSize.Level3)
{
    ...
    const uint32_t dataLength = opts.size.width * opts.size.height * ARGB8888_BYTES;
    uint32_t* data = new uint32_t[dataLength];
    for (uint32_t i = 0; i < dataLength; i++) {
        data[i] = 0xFFFF00FF;
    }
...
HWTEST_F(ImagePixelMapTest, ImagePixelMap052, TestSize.Level3)
{
    ...
    constexpr uint32_t ARGB8888_BYTES = 4;
    const uint32_t dataLength = PIXEL_MAP_TEST_WIDTH * PIXEL_MAP_TEST_HEIGHT * ARGB8888_BYTES;
    uint32_t* data = new uint32_t[dataLength];
    for (uint32_t i = 0; i < dataLength; i++) {
        data[i] = 0x00FF0000;
    }
...
std::unique_ptr<PixelMap> ConstructSLRBigPixmap()
{
    int32_t pixelMapWidth = PIXEL_MAP_BIG_TEST_WIDTH;
    int32_t pixelMapHeight = PIXEL_MAP_BIG_TEST_HEIGHT;
    std::unique_ptr<PixelMap> pixelMap = std::make_unique<PixelMap>();
...
HWTEST_F(ImagePixelMapTest, ImagePixelMapSLR002, TestSize.Level3)
{
    ...
    std::unique_ptr<PixelMap> pixelMap = ConstructSLRBigPixmap();
    EXPECT_NE(pixelMap, nullptr);
...
HWTEST_F(ImagePixelMapTest, ImagePixelMapCreate001, TestSize.Level3)
{
    ...
    const int height = 6; // height
    const int bytes = 4; //bytes
...
/**
* @tc.name: ImagePixelMapCreate002
* @tc.desc: test SLR with DMA
* @tc.type: FUNC
*/
HWTEST_F(ImagePixelMapTest, ImagePixelMapCreate002, TestSize.Level3)
{
    GTEST_LOG_(INFO) << "ImagePixelMapTest: ImagePixelMapCreate002 scale start";
    const int width = 4; // width
    const int height = 6; // height
    const int bytes = 4; //bytes
    std::unique_ptr<uint32_t[]> srcData = std::make_unique<uint32_t[]>(width * height);
    if (srcData == nullptr) {
        GTEST_LOG_(INFO) << "ImagePixelMapTest: ImagePixelMapCreate002  fail.";
        return;
    }
    for (int i = 0; i < width * height; i++) {
        srcData.get()[i] = 0xFF000000;
    ...
    std::unique_ptr<uint8_t[]> colors = std::make_unique<uint8_t[]>(width * height * bytes);
    uint64_t bufferSize = width * height * bytes;
    uint32_t offset = 0;
    uint32_t stride = width * bytes;
    Media::Rect region = {0, 0, width, height};
    auto ret = pixelMap->WritePixels(
        static_cast<uint8_t*>(colors.get()), bufferSize, offset, stride, region);
    ASSERT_EQ(ret, 0);
