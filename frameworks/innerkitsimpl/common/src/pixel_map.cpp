...
bool PixelMap::CopyPixelMap(PixelMap &source, PixelMap &dstPixelMap)
{
    ...
    if (source.GetAllocatorType() == AllocatorType::DMA_ALLOC) {
        ...
        for (int i = 0; i < imageInfo.size.height; ++i) {
            errno_t ret = memcpy_s(dstPixels, source.GetRowBytes(), source.GetPixels() + i * source.GetRowStride(), source.GetRowBytes());
            if (ret != 0) {
            ...
            dstPixels = reinterpret_cast<uint8_t *>(dstPixels) + source.GetRowBytes(); // Move the destination buffer pointer to the next row
        }
    } else {
